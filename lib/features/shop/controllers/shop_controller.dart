...
import '../domain/models/all_seller_model.dart';
...
class ShopController extends ChangeNotifier {
  ...
  AllSellerModel? allSellerModel;
  Future<void> getAllSellerList(bool reload, int offset, {required String type}) async {
    ApiResponse apiResponse = await shopServiceInterface!.getAllSellerList('a', offset);
    log(4);
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      if(offset == 1){

        allSellerModel = null;
        allSellerModel = AllSellerModel.fromJson(apiResponse.response?.data);
      }else{

        allSellerModel?.sellers?.addAll(AllSellerModel.fromJson(apiResponse.response!.data).sellers??[]);
        allSellerModel?.offset  = (AllSellerModel.fromJson(apiResponse.response!.data).offset!);
        allSellerModel?.totalSize  = (AllSellerModel.fromJson(apiResponse.response!.data).totalSize!);
      }

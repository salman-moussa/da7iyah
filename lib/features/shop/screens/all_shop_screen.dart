...
import '../widgets/all_seller_view.dart';

class AllTopSellerScreen extends StatefulWidget {
  final String title;
  final bool isBacButtonExist;

  const AllTopSellerScreen({super.key, required this.title, this.isBacButtonExist = false});
    ...
    return Consumer<ShopController>(
      builder: (context, shopController, _) {
        return Scaffold(backgroundColor: ColorResources.getIconBg(context),
          appBar: CustomAppBar(title: '${getTranslated('all_seller',context)}', isBackButtonExist: widget.isBacButtonExist,
          ...
          body: Padding(padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
            child: AllSellerView(isHomePage: false, scrollController: scrollController,)));
      }
    );
  }
}

...
class BannerController extends ChangeNotifier {
  ...
  Future<void> getBannerList(bool reload) async {
    if (reload || _mainBannerList.isEmpty) {
  ...
  void clickBannerRedirect(BuildContext context, int? id, Product? product, String? type) {
    final categoryList = Provider.of<CategoryController>(context, listen: false).categoryList;
    final brandList = Provider.of<BrandController>(context, listen: false).brandList;
    final sellerList = Provider.of<ShopController>(context, listen: false).allSellerModel?.sellers;

    if (type == 'category' && categoryList.isNotEmpty) {
      final cIndex = categoryList.indexWhere((element) => element.id == id);
      if (cIndex != -1 && categoryList[cIndex].name != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => BrandAndCategoryProductScreen(
          isBrand: false,
          id: id.toString(),
          name: '${categoryList[cIndex].name}',
        )));
      }
    } else if (type == 'product' && product != null) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => ProductDetails(
        productId: product.id,
        slug: product.slug,
      )));
    } else if (type == 'brand' && brandList.isNotEmpty) {
      final bIndex = brandList.indexWhere((element) => element.id == id);
      if (bIndex != -1 && brandList[bIndex].name != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => BrandAndCategoryProductScreen(
          isBrand: true,
          id: id.toString(),
          name: '${brandList[bIndex].name}',
        )));
      }
    } else if (type == 'shop' && sellerList != null && sellerList.isNotEmpty) {
      final tIndex = sellerList.indexWhere((element) => element.id == id);
      if (tIndex != -1 && sellerList[tIndex].shop?.name != null) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => TopSellerProductScreen(
          sellerId: id,
          temporaryClose: sellerList[tIndex].shop?.temporaryClose,
          vacationStatus: sellerList[tIndex].shop?.vacationStatus,
          vacationEndDate: sellerList[tIndex].shop?.vacationEndDate,
          vacationStartDate: sellerList[tIndex].shop?.vacationStartDate,
          name: sellerList[tIndex].shop?.name,
          banner: sellerList[tIndex].shop?.banner,
          image: sellerList[tIndex].shop?.image,
        )));
      }
    }
  }
